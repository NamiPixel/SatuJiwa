...
<head>
    ...
    <title>SatuJiwa — Lupa Kata Laluan</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent }
        ...
        .card { width: 100%; max-width: 420px; background: rgba(255, 255, 255, 0.78); border: 1px solid rgba(201, 120, 138, 0.2); border-radius: 24px; padding: 48px 40px; backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px); box-shadow: 0 32px 80px rgba(201, 120, 138, 0.14); animation: cardIn .8s cubic-bezier(.22, .68, 0, 1.2) both; position: relative; overflow: hidden; }
        ...
        .input { width: 100%; background: var(--input-bg); border: 1px solid var(--border); border-radius: 12px; padding: 13px 14px 13px 44px; color: var(--text-dark); font-size: 13px; font-family: 'Nunito', sans-serif; font-weight: 400; outline: none; transition: all .25s; }
        .input:focus { border-color: var(--rose); background: rgba(255, 255, 255, 0.92); box-shadow: 0 0 0 3px rgba(201, 120, 138, 0.12); }
        .btn-primary { width: 100%; background: linear-gradient(135deg, #C9788A 0%, #B05A70 100%); border: none; border-radius: 50px; padding: 15px; color: #fff; font-weight: 700; font-size: 12px; letter-spacing: 1px; text-transform: uppercase; cursor: pointer; transition: all .3s; box-shadow: 0 6px 24px rgba(201, 120, 138, 0.3); margin-top: 10px; }
        .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 12px 32px rgba(201, 120, 138, 0.4); }
        .back-link { display: block; text-align: center; margin-top: 24px; font-size: 13px; color: var(--rose); text-decoration: none; transition: color 0.2s; }
        .back-link:hover { color: var(--text-dark); }
        .alert { padding: 12px 16px; border-radius: 12px; font-size: 13px; margin-bottom: 20px; line-height: 1.5; }
        .alert-success { background: rgba(74,140,106,0.08); color: #4A8C6A; border: 1px solid rgba(74,140,106,0.2); text-align:center; }
        .alert-error { background: rgba(192,80,96,0.08); color: #C05060; border: 1px solid rgba(192,80,96,0.2); text-align:center; }
...
<body>
    ...
    <div class="page">
        <div class="card">
            ...
            <h2 style="font-family:'Cormorant Garamond', serif; font-size: 22px; color: var(--text-dark); margin-bottom: 8px; text-align: center;">Lupa Kata Laluan?</h2>
            <p style="font-size: 13px; color: var(--text-soft); text-align: center; margin-bottom: 32px; line-height: 1.6;">Masukkan emel anda dan kami akan hantar pautan untuk menetapkan semula kata laluan anda.</p>
            ...
            <form method="POST" action="{{ route('password.email') }}">
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrap">
                        <input type="email" name="email" class="input" value="{{ old('email') }}" required autofocus placeholder="nama@email.com">
                ...
                <button type="submit" class="btn-primary">Hantar Pautan Reset</button>
            </form>

            <a href="{{ route('login') }}" class="back-link">← Kembali ke Log Masuk</a>
        </div>
    </div>
    <script>
